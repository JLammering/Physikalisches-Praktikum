...
d_winkel60, delta_nuwinkel60= np.genfromtxt('daten/v_winkel60.txt',unpack = 'True')

#winkel 60
v_winkel60 = (delta_nuwinkel60/(2*nu_0*np.cos(alpha[2])))*c
#print('v_winkel60 = ', v_winkel60)
x = v_winkel60
y =  delta_nuwinkel60/np.cos(alpha[0])

m, m_err, y_achs, y_errachs = linregress(x, y)
#print('m= ',m, '2 nu_0/c =', 2*nu_0/c, 'm_err= ',m_err)

np.savetxt('build/linv_winkel60.txt', np.column_stack([m, m_err, y_achs, y_errachs]),
header="Steigung, Fehler Steigung, y-Achsenab, Fehler y-Achsenab")
np.savetxt('build/v_winkel60.txt', np.column_stack([v_winkel60]),
header="v_winkel60")
...
plt.savefig('build/plotvwinkel60.pdf')
