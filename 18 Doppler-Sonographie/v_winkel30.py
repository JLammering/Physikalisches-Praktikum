...
d_winkel30, delta_nuwinkel30= np.genfromtxt('daten/v_winkel30.txt',unpack = 'True')

#winkel 30
v_winkel30 = (delta_nuwinkel30/(2*nu_0*np.cos(alpha[1])))*c
#print('v_winkel15 = ', v_winkel15)
x = v_winkel30
y =  delta_nuwinkel30/np.cos(alpha[0])

m, m_err, y_achs, y_errachs = linregress(x, y)
#print('m= ',m, '2 nu_0/c =', 2*nu_0/c, 'm_err= ',m_err)

np.savetxt('build/linv_winkel30.txt', np.column_stack([m, m_err, y_achs, y_errachs]),
header="Steigung, Fehler Steigung, y-Achsenab, Fehler y-Achsenab")
np.savetxt('build/v_winkel30.txt', np.column_stack([v_winkel30]),
header="v_winkel30")
...
plt.savefig('build/plotvwinkel30.pdf')
